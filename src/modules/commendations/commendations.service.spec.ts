...
describe('CommendationsService', () => {
  ...
  beforeEach(async () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            uploadFile: jest
              .fn()
              .mockResolvedValue({ objectName: 'uploaded/path.pdf' }),
          },
        },
      ],
    }).compile();
    ...
    commendationRepo = module.get<Repository<Commendation>>(
      getRepositoryToken(Commendation),
    );
  ...
  describe('update', () => {
    it(' nên cập nhật thông tin và upload file mới', async () => {
      (commendationRepo.findOne as jest.Mock).mockResolvedValue({
        ...mockCommendation,
      });
      ...
      expect(minioService.uploadFile).toHaveBeenCalled();
      expect(commendationRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Tiêu đề mới',
          decisionFileUrl: 'uploaded/path.pdf',
        }),
    ...
    it(' nên ném lỗi NotFoundException nếu bản ghi khen thưởng không tồn tại', async () => {
      ...
      await expect(service.update(mockCommendationId, {})).rejects.toThrow(
        NotFoundException,
      );
    });
  });
