...
export class DocumentChunkerService {
  ...
  private parseHeading(line: string): HeadingInfo | null {
    ...
    const chapterMatch = normalized.match(
      /^CHƯƠNG\s+([IVXLCDM0-9A-ZĂÂĐÊÔƠƯ]+)/iu,
    );
    ...
    if (
      ...
      this.isSafeBulletHeading(normalized)
    ) {
      return {
        level: 6,
        text: normalized,
  ...
  private sanitizeSectionPath(value?: string | null): string | undefined {
    ...
    if (cleaned.length > DocumentChunkerService.MAX_SECTION_PATH_LENGTH) {
      return cleaned
        .slice(0, DocumentChunkerService.MAX_SECTION_PATH_LENGTH)
        .trim();
    }
  ...
  private isListHeavyParagraph(text: string): boolean {
    ...
    const listLikeCount = lines.filter((line) =>
      this.isListLikeLine(line),
    ).length;
  ...
  private splitLargeBlock(
    ...
    for (const sentence of sentences) {
      ...
      if (sentenceWords > maxWords) {
        flush();
        chunks.push(
          ...this.slidingWindowByWords(sentence, maxWords, overlapWords),
        );
        continue;
      }
  ...
  private mergeSmallNeighborChunks(
    ...
    for (const chunk of chunks) {
      ...
      if (
        ...
        (currentWords < minWords ||
          this.countWords(previous.content) < minWords) &&
    ...
    return merged.map((item) => ({
      ...
      metadata: {
        ...(item.metadata ?? {}),
        heading:
          this.sanitizeSectionPath(
            typeof item.metadata?.heading === 'string'
