...
describe('AdmissionApplicationService (Applicant Logic)', () => {
  ...
  describe('initAdmissionForQCUT', () => {
    ...
    it(' Nên tạo mới hồ sơ và 7 bước nếu chưa có hồ sơ nào', async () => {
      ...
      expect(stepRepo.create).toHaveBeenCalledWith(expect.arrayContaining([
        expect.objectContaining({ stepOrder: 1, stepCode: AdmissionWorkflowStep.APPLICATION }),
        expect.objectContaining({ stepOrder: 7 })
      ]));
    });

    it(' Nên trả về hồ sơ cũ nếu đang trong quá trình xử lý', async () => {
      const existingApp = { id: 'old-app', overallStatus: AdmissionOverallStatus.DRAFT };
  ...
  describe('saveDraftStep', () => {
    ...
    it(' Không phải QCUT/OUTSTANDING_INDIVIDUAL thì không được lưu', async () => {
      ...
      await expect(service.saveDraftStep(badUser, stepCode, { formData: { a: 1 } }))
        .rejects.toThrow(ForbiddenException);
    });

    it(' Lưu nháp lần đầu (chưa có submission) - tạo mới version 1', async () => {
      applicationRepo.findOne.mockResolvedValue({ id: 'app-1', overallStatus: AdmissionOverallStatus.DRAFT });
      stepRepo.findOne.mockResolvedValue({ id: 's1', isCurrent: true, status: AdmissionWorkflowStepStatus.NOT_STARTED });
      submissionRepo.findOne.mockResolvedValue(null); // Chưa có submission
      submissionRepo.create.mockReturnValue({ version: 1 });

      await service.saveDraftStep(user, stepCode, { formData: { name: 'Anh Vu' } });

      expect(submissionRepo.create).toHaveBeenCalledWith(expect.objectContaining({
        version: 1,
    ...
    it(' Đã có bản nháp chưa submit - nên thực hiện MERGE dữ liệu', async () => {
      ...
      stepRepo.findOne.mockResolvedValue({ id: 's1', isCurrent: true, status: AdmissionWorkflowStepStatus.IN_PROGRESS });
      
      const oldSubmission = { id: 'sub-1', formData: { field1: 'val1' }, submittedAt: null };
      submissionRepo.findOne.mockResolvedValue(oldSubmission);

      await service.saveDraftStep(user, stepCode, { formData: { field2: 'val2' } });

      // Kiểm tra việc merge: {field1: 'val1', field2: 'val2'}
      expect(oldSubmission.formData).toEqual({ field1: 'val1', field2: 'val2' });
  ...
  describe('submitStep', () => {
    ...
    it(' Thiếu giấy tờ bắt buộc theo quy định của step', async () => {
      applicationRepo.findOne.mockResolvedValue({ id: 'app-1', currentStepCode: AdmissionWorkflowStep.APPLICATION });
      ...
      await expect(service.submitStep(user, AdmissionWorkflowStep.APPLICATION, { formData: {} }))
    ...
    it(' Submit thành công - phải khóa step hiện tại và mở step tiếp theo', async () => {
      ...
      stepRepo.findOne.mockResolvedValueOnce(currentStep) // Lần 1 cho getMyStepOrFail
      ...
      await service.submitStep(user, AdmissionWorkflowStep.APPLICATION, { formData: validFormData });
    ...
    it(' Submit bước cuối cùng - Hồ sơ phải chuyển sang APPROVED', async () => {
      ...
      applicationRepo.findOne.mockResolvedValue({ id: 'app-1', currentStepCode: AdmissionWorkflowStep.LOCAL_VERIFICATION });
      stepRepo.findOne.mockResolvedValue({
        id: 's4',
        stepOrder: 4,
        isCurrent: true,
