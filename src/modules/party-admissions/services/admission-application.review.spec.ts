...
describe('AdmissionApplicationService (Review Logic)', () => {
  ...
  const mockManager = {
    getRepository: jest.fn().mockImplementation((entity) => {
      ...
      if (entity === PartyAdmissionStepReviewEntity) return { create: jest.fn((d) => d), save: jest.fn() };
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(PartyAdmissionApplicationEntity), useValue: { findOne: jest.fn(), save: jest.fn() } },
        { provide: getRepositoryToken(PartyAdmissionStepEntity), useValue: { findOne: jest.fn(), save: jest.fn() } },
        { provide: getRepositoryToken(PartyAdmissionStepSubmissionEntity), useValue: { findOne: jest.fn(), create: jest.fn(), save: jest.fn() } },
        ...
        {
          provide: DataSource,
          useValue: { transaction: jest.fn().mockImplementation((cb) => cb(mockManager)) },
        },
      ],
    }).compile();

    service = module.get<AdmissionApplicationService>(AdmissionApplicationService);
    applicationRepo = module.get(getRepositoryToken(PartyAdmissionApplicationEntity));
    stepRepo = module.get(getRepositoryToken(PartyAdmissionStepEntity));
    submissionRepo = module.get(getRepositoryToken(PartyAdmissionStepSubmissionEntity));
  ...
  describe('approveStep', () => {
    ...
    it(' Duyệt thành công và chuyển sang step tiếp theo', async () => {
      applicationRepo.findOne.mockResolvedValue({ id: appId, currentStepCode: AdmissionWorkflowStep.CHI_UY_REVIEW });
      
      const currentStep = { id: 's2', stepOrder: 2, stepCode: AdmissionWorkflowStep.CHI_UY_REVIEW };
      const nextStep = { id: 's3', stepOrder: 3, stepCode: AdmissionWorkflowStep.PBT_CONTENT_REVIEW };
      
      stepRepo.findOne.mockResolvedValueOnce(currentStep).mockResolvedValueOnce(nextStep);

      const result = await service.approveStep(appId, reviewerId, { stepCode: AdmissionWorkflowStep.CHI_UY_REVIEW, note: 'OK' });
    ...
    it(' Duyệt bước cuối cùng - Hồ sơ thành APPROVED và thăng cấp User Role', async () => {
      ...
      roleRepo.findOne.mockResolvedValue({ id: 'party-member-id', name: 'PARTY_MEMBER' });

      const result = await service.approveStep(appId, reviewerId, { stepCode: AdmissionWorkflowStep.SECRETARY_RESOLUTION_REVIEW });

      expect(result.done).toBe(true);
      expect(userRepo.save).toHaveBeenCalledWith(expect.objectContaining({ roleId: 'party-member-id' }));
    });

    it(' Sai step hiện tại (Current Step Code mismatch)', async () => {
        applicationRepo.findOne.mockResolvedValue({ id: appId, currentStepCode: AdmissionWorkflowStep.APPLICATION });
        
        await expect(service.approveStep(appId, reviewerId, { stepCode: AdmissionWorkflowStep.CHI_UY_REVIEW }))
  ...
  describe('returnStep', () => {
    it(' Trả hồ sơ về bước trước đó hợp lệ', async () => {
      ...
      const current = { id: 's3', stepOrder: 3, stepCode: AdmissionWorkflowStep.PBT_CONTENT_REVIEW };
      const target = { id: 's1', stepOrder: 1, stepCode: AdmissionWorkflowStep.APPLICATION };

      stepRepo.findOne.mockResolvedValueOnce(current).mockResolvedValueOnce(target);
    ...
    it(' Trả về bước phía sau hoặc bằng bước hiện tại (Invalid Order)', async () => {
        ...
        await expect(service.returnStep('app-1', 'rev-1', { 
  ...
  describe('rejectStep', () => {
    it(' Từ chối hồ sơ - Trạng thái tổng chuyển sang REJECTED', async () => {
      const app = { id: 'app-1', overallStatus: AdmissionOverallStatus.IN_PROGRESS };
      applicationRepo.findOne.mockResolvedValue(app);
      stepRepo.findOne.mockResolvedValue({ id: 's1' });

      await service.rejectStep('app-1', 'rev-1', { stepCode: AdmissionWorkflowStep.APPLICATION, reason: 'Không đủ tiêu chuẩn' });
  ...
  describe('submitResolutionDraft', () => {
    it(' Role không phải COMMITTEE_MEMBER thì không được nộp nghị quyết', async () => {
      ...
      await expect(service.submitResolutionDraft('app-1', user, { formData: {} }))
    ...
    it(' Chi uỷ nộp nghị quyết - chuyển step sang Duyệt nghị quyết', async () => {
      ...
      const currentStep = { id: 's6', stepOrder: 6, isCurrent: true, stepCode: AdmissionWorkflowStep.RESOLUTION_DRAFTING };
      ...
      const dto = { formData: { [AdmissionDocumentType.NGHI_QUYET_KET_NAP_DU_THAO]: 'file_url' } };
