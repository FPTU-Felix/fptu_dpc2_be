...
describe('AdmissionApplicationService (Queries)', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(PartyAdmissionApplicationEntity), useFactory: mockRepo },
        { provide: getRepositoryToken(PartyAdmissionStepEntity), useFactory: mockRepo },
        { provide: getRepositoryToken(PartyAdmissionStepSubmissionEntity), useFactory: mockRepo },
        { provide: getRepositoryToken(PartyAdmissionStepReviewEntity), useFactory: mockRepo },
  ...
  describe('getMyCurrentStatus', () => {
    ...
    it(' Nên trả về trạng thái hồ sơ đầy đủ khi hồ sơ tồn tại', async () => {
      const mockApp = { id: 'app-1', outstandingIndividualId: userId, overallStatus: AdmissionOverallStatus.IN_PROGRESS };
      ...
      expect(applicationRepo.findOne).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it(' Nên throw NotFoundException nếu người dùng không có hồ sơ', async () => {
      ...
      await expect(service.getMyCurrentStatus(userId))
    ...
    it(' Nếu người dùng có nhiều hồ sơ, phải lấy hồ sơ mới nhất (DESC)', async () => {
      ...
      applicationRepo.findOne = jest.fn().mockResolvedValue({ id: 'latest-app' });
      stepRepo.find = jest.fn().mockResolvedValue([]);
  ...
  describe('getMyCurrentStatusWithRoleCheck', () => {
    it(' Role không hợp lệ (ví dụ: ADMIN) nên bị Forbidden', async () => {
      ...
      await expect(service.getMyCurrentStatusWithRoleCheck(user))
    ...
    it(' Role hợp lệ (QCUT) nên gọi tiếp getMyCurrentStatus', async () => {
      ...
      const spy = jest.spyOn(service, 'getMyCurrentStatus').mockResolvedValue({ applicationId: 'app-1' } as any);
      userRepo.findOne = jest.fn().mockResolvedValue({ id: 'u1', role: { name: 'QCUT' } });
  ...
  describe('getApplicationList', () => {
    ...
    beforeEach(() => {
      ...
      applicationRepo.createQueryBuilder = jest.fn().mockReturnValue(mockQueryBuilder);
    });
    ...
    it(' COMMITTEE_MEMBER chỉ thấy các bước CHI_UY_REVIEW và RESOLUTION_DRAFTING', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ 
          processableSteps: expect.arrayContaining(['CHI_UY_REVIEW', 'RESOLUTION_DRAFTING']) 
        })
      );
    });
